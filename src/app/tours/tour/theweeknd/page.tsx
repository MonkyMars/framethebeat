...
export default function Page() {
  ...
  return (
    <>
      {showIntro && (
        <section className="cinematicIntro">
          <h1>The Trilogy Immersion</h1>
          <p>Embark on a deep dive into The Weeknd&apos;s visionary mixtapes.</p>
        </section>
      )}
      {!showIntro && (
        <>
          ...
          <main className="timeline">
            <header className="timelineHeader">
              <h1>Three Nights, One Story: The Trilogy Experience</h1>
            </header>
            <VerticalTimeline>
...
const TimelineElement = ({ item }: TimelineElementProps) => {
  return (
    <article className="timelineElement">
      <header>
        <h1>{item.title}</h1>
      </header>
      <p>{item.description}</p>
      {item.image?.src && <Image src={item.image.src} alt={item.image.alt} width={1000} height={1000} />}
    </article>
  );
};
